...
parser = JsonOutputToolsParser(return_id=True) #it is going to return us te function call we got back from the llm and transform it into a dictionary
parser_pydantic = PydanticToolsParser(tools=[AnswerQuestion]) # it is going to searech for the function calling invocation and it's going to parse iut and transform it into an answer querstion object
